...
let intervarlHandle = null
function displayRequest(requestContainer) {
	intervarlHandle = createHeightInterval()
...
function closeRequest() {
	...
	if (totalRequests > 0)
		...
		clearInterval(intervarlHandle)
}

function createHeightInterval() {
	clearInterval(intervarlHandle)
	let id = setInterval(() => { console.log('height'); displayWidget(document.getElementById('request-' + (totalRequests - 1)).clientHeight) }, 10);
...
/* window.onDomChange(function () {
	console.log(totalRequests, 'request-' + (totalRequests - 1))
	if (totalRequests > 0)
		displayWidget(document.getElementById('request-' + (totalRequests - 1)).clientHeight)
});
 */